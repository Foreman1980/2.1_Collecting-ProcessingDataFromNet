...
class InstagramSpider(scrapy.Spider):
    ...
    def user_data_parse(self, response: HtmlResponse, user_name):
        full_name = response.xpath('//h1/text()').extract_first()
        profil_pic_url = response.xpath('//main//img[@data-testid="user-avatar"]/@src').extract_first()
        user_id = self.fetch_user_id(response.text, user_name)
        # 1 '{"id":"8995164690","include_reel":true,"fetch_mutual":true,"first":24}'
        ...
        yield response.follow(
            ...
            cb_kwargs={'user_name': user_name,
                       'full_name': full_name if full_name else 'No data',
                       'user_id': user_id,
                       'profil_pic_url': profil_pic_url,
                       'variables': deepcopy(var_followers),
                       }
        )

        # # 1 '{"id":"8995164690","include_reel":true,"fetch_mutual":false,"first":24}'
        # var_followings = {'id': user_id,
        #                   'include_reel': True,
        #                   'fetch_mutual': False,
        #                   'first': 24,
        #                   }
        # # 2 '{"id":"8995164690","include_reel":true,"fetch_mutual":false,"first":12,"after":"QVFBLXFDZ0RTSkVEdG1uMVQ3LTNtMkRNMHM2Z1h5Qks0UlpOZmwzeDRVWHJUa3dHZ2tPQkdLTm5NcG8wcWp2OG81T3o3REFWVTNYZzg1R0F3VklFZ2RIRw=="}'
        # url_followers = f'{self.graphql_url}query_hash={self.followers_hash}&{urlencode(var_followers)}'
        # yield response.follow(
        #     url_followers,
        #     callback=self.user_followers_parse,
    ...
    def user_followers_parse(self, response: HtmlResponse, user_name, full_name, user_id, profil_pic_url, variables):
        ...
        for follower in followers:
            # followers_list += follower.get('node').get('username')
            yield InstaparserItem(
                user_name=follower.get('node').get('username'),
                full_name=follower.get('node').get('full_name'),
                user_id=follower.get('node').get('id'),
                profil_pic_url=follower.get('node').get('profile_pic_url'),
                following=[user_name],
                followers=[]
            )

    # def user_followings_parse(self, response: HtmlResponse, user_name, user_id, variables):
    #     j_data = json.loads(response.text)
    #     pass
    #
